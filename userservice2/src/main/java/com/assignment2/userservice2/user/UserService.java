...
import com.assignment2.userservice2.entities.EmailsChangeMsg;
...
import com.assignment2.userservice2.errorhandling.AlreadyExistsException;
import com.assignment2.userservice2.errorhandling.NotFoundException;
...
public class UserService {
    ...
    public UserAddressChangeResponse updateUserAddress(int userId, DeliveryAddress deliveryAddress) {
        ...
        if(user.isPresent()){
            ...
            User savedUser = saveUser(fetchUser);
            return new UserAddressChangeResponse(savedUser.getUserId(), savedUser.getDeliveryAddress());
        }
        throw new NotFoundException("User with ID: "+userId+" not found");
    }

    public UserEmailChangeResponse updateEmailAddress(int userId, String oldEmail, String newEmail) {
        Optional<User> user = userRepository.findById(userId);
            ...
            if(emailList.length > 0){
                for(int i=0;i<emailList.length;i++){
                    if(emailList[i].equals(oldEmail)){
                        ...
                        User savedUser = saveUser(fetchUser);
                        return new UserEmailChangeResponse(savedUser.getUserId(), savedUser.getEmails());
                    }
                }
